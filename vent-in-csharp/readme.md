...
* `Entities`, an entity is any object implementing the `IEntity` interface which consists of an `int Id` and implements the `ICloneable` interface. Entities which don't need much more than a `MemberwiseClone` can derive from the `EntityBase` class.

* `EntityStore`, an object containing all objects relevant to the application and offering operations on these objects. Operations include:
  * registration and deregistration of new entities,
  * notifying the store of changes to entities
  * undo / redo to get the state of the application as contained in a previous/future state.
...
* `Head state`, the "head state" of an entity is the current working version the application may have of an entity. The application can use and change this version as needed. However the EntityStore can change this version as well when undo/redo or deregister is called on the store. Think of it as the current file version in git, it can be changed as needed, but when  the changes are reverted, git will change the contents of this version.

The underlying assumption is that all objects managed by the EntityStore are either entities OR objects which are fully owned by an entity. References in entities to objects outside the EntityStore or objects contained in other entities are not forbidden but may introduce side-effects and are discouraged.
...
We can also `Register` an entity to the store. Registering an entity will add the entity, but does not create versioning information. This means  the registered entity will not be affected by `Undo`/`Redo` operations. `Reverting` an entity without version information will result in a `ContractException` in Debug builds or a NOP in Release builds.
...
* Deregister non versioned entities. Versioned entities should not be removed as this will put the store in an invalid state as deregistering will not take in account the (potential) relation with mutations.
